...
tool_exec <- function(in_params, out_params)
{
	...
	var.nc <- brick(source_nc, varname=var, layer="time")
	...
	if(time_average=="Daily Averages"){
		...
		for(DAY in days){
			...
			av.var <- calc(sub.var, fun=mean, filename=paste0(out_folder,"/",var,"_Day",DAY,".tif"))
		...
		for(MONTH in months){
			...
			av.var <- calc(sub.var, fun=mean, filename=paste0(out_folder,"/",var,"_Month",MONTH,".tif"))
		...
		for(YEAR in years){
			...
			av.var <- calc(sub.var, fun=mean, filename=paste0(out_folder,"/",var,"_Year",YEAR,".tif"))
		...
		for(HOUR in hours){
			...
			av.var <- calc(sub.var, fun=mean, filename=paste0(out_folder,"/",var,"_Hour",HOUR,".tif"))
